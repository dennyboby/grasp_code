...
def plot_results(
    ...
    :return:
    """
    gs = detect_grasps(grasp_q_img, grasp_angle_img, width_img=grasp_width_img, length_img=grasp_length_img, no_grasps=no_grasps)
    ...
    if rgb_img is not None:
        ...
        ax.imshow(grasp_img, cmap='gray')
    if grasp_q_img.shape == grasp_img.shape[:2]:
        for g in gs:
            g.plot(ax, color='red')
    else:
        factor = (1080 / grasp_q_img.shape[0], 1920 / grasp_q_img.shape[1])
        for g in gs:
            gt = g.as_gt
            print(gt.corner, gt.angle, gt.length, gt.width)
            gt.scale(factor)
            gt.plot(ax, color='red')
    ax.set_title('Grasp')
    ax.axis('off')
    ...
    plot = ax.imshow(grasp_angle_img, cmap='hsv', vmin=-np.pi, vmax=np.pi)
    ax.set_title('Angle')
    ax.axis('off')
    plt.colorbar(plot)

    ax = fig.add_subplot(2, 3, 6)
    plot = ax.imshow(grasp_width_img, cmap='jet', vmin=0, vmax=50)
    ax.set_title('Width')
    ax.axis('off')
    plt.colorbar(plot)
...
def plot_grasp(
    ...
    ax = plt.subplot(111)
    ax.imshow(rgb_img)
    if grasp_q_img.shape == rgb_img.shape[:2]:
        for g in gs:
            g.plot(ax)
    else:
        factor = (1080 / grasp_q_img.shape[0], 1920 / grasp_q_img.shape[1])
