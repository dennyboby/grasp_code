...
from .grasp import GraspTriangles, detect_grasps
...
def calculate_iou_match(grasp_q, grasp_angle, ground_truth_bbs, no_grasps=1, grasp_width=None, grasp_length=None, threshold=0.25):
    """
    Calculate grasp success using the IoU (Jacquard) metric (e.g. in https://arxiv.org/abs/1301.3592)
    A success is counted if grasp triangle has a 25% IoU with a ground truth, and is withing 30 degrees.
    ...
    if not isinstance(ground_truth_bbs, GraspTriangles):
        gt_bbs = GraspTriangles.load_from_array(ground_truth_bbs)
    else:
        gt_bbs = ground_truth_bbs
    gs = detect_grasps(grasp_q, grasp_angle, width_img=grasp_width, length_img=grasp_length, no_grasps=no_grasps)
    for g in gs:
